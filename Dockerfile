...
CMD ["sh", "-c", "echo 'Running migrations...' && bundle exec rake db:migrate 2>&1 || echo 'Migration issue (non-fatal, will retry on next deploy)' && echo 'Starting Puma...' && bundle exec puma -C config/puma.rb -b tcp://0.0.0.0:${PORT:-3000}"]
